# VRS1b2 (estimate incidence with IncidencePrevalence)
#
# 1. ---- Initial settings ----
...
targetCohortTable <- read_csv(file_name("targetCohortTable", name), col_types = cols(
  ...
  mutate(cohort_end_date = if_else(
    ...
    pmin(cohort_end_date, cohort_start_date + days(90)),
    cohort_end_date
  ))
...
result_file_name <- paste0(subdir, municipality_id, "_VRS1b2_lastid_", municipality_name, ".csv")
...
for (i in 1:4) {
  ...
  result_file_name <- paste0(subdir, municipality_id, "_VRS1b2_", result_name, "_", municipality_name, ".csv")
  write_csv(inc_df, result_file_name)
}
...
result_file_name <- paste0(subdir, municipality_id, "_VRS1b2_duration_", municipality_name, ".csv")
